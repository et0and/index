...
import { TwitterBookmarkIcon, TwitterChatBubbleIcon, TwitterHeartIcon, TwitterMenuIcon, TwitterRetweetIcon } from '@/components/icons'
			...
			<div className="w-[500px] rounded-[24px] border p-[24px] shadow">
				<div className="flex flex-col gap-4">
					...
					<div className="flex gap-1 text-muted-foreground">
						...
						<div>⋅</div>
						<div>
							<span className="font-bold text-black">{millify(views, { precision: 1, lowercase: true })}</span> Views
						</div>
					</div>
					...
					<div className="flex justify-between fill-gray-500 text-muted-foreground">
						...
						<TooltipProvider>
							<Tooltip>
								...
								<TooltipContent sideOffset={8} className="flex  flex-col gap-6 rounded-[24px] border bg-white p-4 align-top shadow">
									{USERS.map((user) => (
										<div key={user.id} className="flex gap-2 align-top">
											<Image width={40} height={40} src={user.avatar} alt={user.username} className="h-10 w-10 rounded-full" />
